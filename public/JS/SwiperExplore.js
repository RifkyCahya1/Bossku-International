document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!$current || !$total || !$bar) {
        console.warn(
            "Progress elements not found (currentSlide / totalSlide / progressBar)."
        );
        return;
    }

    const swiper = new Swiper(".ExploreSlider", {
        slidesPerView: 1,
        spaceBetween: 12,
        loop: true,
        centeredSlides: true,
    ...
    swiper.on("slideChangeTransitionEnd", () => {
        ...
        const active = document.querySelector(
            ".ExploreSlider .swiper-slide-active"
        );
        if (active) active.classList.add("is-center");
    });
