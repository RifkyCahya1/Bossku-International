...
<section class="relative text-white py-12 md:py-20 overflow-hidden">
    ...
    <div class="absolute inset-0">
        ...
        <div class="absolute inset-0 bg-gradient-to-br from-black/100 via-black/90 to-black/70"></div>
    </div>
    ...
    <div class="relative container mx-auto px-4 md:px-16 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
        ...
        <div class="space-y-6 text-center md:text-left order-1 md:order-1">
            ...
            <p class="text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed max-w-xl mx-auto md:mx-0">
            ...
            <div class="mt-8 sm:mt-10 space-y-2">
                <div class="flex items-end justify-center md:justify-start font-semibold gap-1">
                    ...
                    <span id="totalSlide" class="text-gray-400 text-base sm:text-lg font-medium">03</span>
                </div>
                ...
                <div class="flex items-center justify-center md:justify-end gap-2">
                    <button id="prevBtn"
                        class="p-2.5 sm:p-3 border border-gray-400 rounded-full hover:bg-[#CB0001]  hover:border-[#CB0001] transition-all duration-300">
...
<script type="module">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Initialize by selector string to let Swiper handle element lookup robustly
        ...
        // Fallback-safe slide count getter: prefer swiper.slides when present, otherwise query DOM
        function countOriginalSlides() {
            const slidesArray = (swiper && swiper.slides && swiper.slides.length)
                ? Array.from(swiper.slides)
                : Array.from((swiperEl || document).querySelectorAll('.swiper-slide'));
            return slidesArray.filter(s => !s.classList.contains('swiper-slide-duplicate')).length;
        }

        function refreshProgress() {
            const total = Math.max(1, countOriginalSlides());
            const idx = (swiper && (swiper.realIndex ?? swiper.activeIndex)) ?? 0;
        ...
        // Update progress on init/slide/resizes
        swiper.on('init slideChange resize', refreshProgress);

        // Ensure initial progress runs (some builds may not emit init after construction)
        refreshProgress();
    });
</script>
@endsection
